...
- 前端：`spring-ai-frontend-vue/src/components/ChatInput.vue`（TTS处理）
- 前端：`spring-ai-frontend-vue/src/components/ChatMessages.vue`（音频显示和下载）
...
<a :href="item.content" download="generated_audio.wav" class="download-btn">
  下载音频
</a>
```
...
解决方案：
1. 使用 **FileReader API** 将音频Blob转换为base64编码的data URL
2. FileReader比`btoa()`更可靠，能正确处理二进制数据
3. `<audio>`标签原生支持data URL且不需要Range请求
4. 添加 `preload="auto"` 属性确保音频预加载
5. 添加下载按钮，方便用户保存音频文件
...
**已实施优化**：在 `docker-compose.yml` 中添加了DJL缓存卷挂载：

```yaml
volumes:
  - djl-cache:/root/.djl.ai  # 持久化PyTorch库缓存
...
**效果：**
- ✅ **首次启动**：需要下载PyTorch库（约200-300MB），耗时5-10分钟
- ✅ **后续启动**：直接使用缓存的库，启动只需几秒钟
- ✅ **重建容器**：不需要重新下载，缓存保留在Docker卷中
- ✅ **节省流量**：避免每次重建都下载几百MB的文件
